...
public interface ContractRepository extends JpaRepository<Contract, Long>, QueryDslPredicateExecutor<Contract> {
    @Query("SELECT c FROM Contract c LEFT JOIN FETCH c.relatedDepartments LEFT JOIN FETCH c.contractParty " +
        "LEFT JOIN FETCH c.category LEFT JOIN FETCH c.projects LEFT JOIN FETCH c.tasks WHERE c.id = (:id)")
    ...
    @Query("SELECT c FROM Contract c LEFT JOIN FETCH c.attachments WHERE c.id = (:id)")
    Contract findByIdAndFetchAttachments(@Param("id") Long id);
}
